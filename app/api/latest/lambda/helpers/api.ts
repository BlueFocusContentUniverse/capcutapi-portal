...
const makeRequest = async <Res>(
  ...
  try {
    ...
    if (!result.ok) {
      ...
      throw new Error(
        `HTTP request failed with status ${result.status}: ${result.statusText}`,
      );
    }
...
export const renderVideo = async ({
  ...
  if (inputProps.overlays) {
    ...
    console.log(
      "Overlays details:",
      JSON.stringify(inputProps.overlays, null, 2),
    );
  }
  ...
  if (inputProps.durationInFrames) {
    ...
    console.log(
      "Duration in seconds:",
      inputProps.durationInFrames / (inputProps.fps || 30),
    );
  }
  ...
  if (inputProps.width && inputProps.height) {
    console.log(
      "Video dimensions:",
      `${inputProps.width}x${inputProps.height}`,
    );
    console.log(
      "Aspect ratio:",
      (inputProps.width / inputProps.height).toFixed(2),
    );
  }
  ...
  if (response && typeof response === "object") {
    ...
    if ("type" in response) {
      ...
      if (response.type === "error") {
        ...
        console.log(
          "Progress percentage:",
          `${(response.progress * 100).toFixed(2)}%`,
        );

...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!renderState) {
      console.log(
        `🔄 [${new Date().toISOString()}] Render state not found for ${validatedData.id}, trying once more... (first attempt took ${firstAttemptDuration.toFixed(2)}ms)`,
      );
      ...
      console.log(
        `🔄 [${new Date().toISOString()}] Second attempt result: ${renderState ? "FOUND" : "NOT FOUND"} (took ${secondAttemptDuration.toFixed(2)}ms)`,
      );
    } else {
      console.log(
        `✅ [${new Date().toISOString()}] Render state found on first attempt for ${validatedData.id} (took ${firstAttemptDuration.toFixed(2)}ms)`,
      );
    }

    if (!renderState) {
      console.log(
        `🚨 Final failure: No render state found for ${validatedData.id}`,
      );

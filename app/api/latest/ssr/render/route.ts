...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `🎬 [${new Date().toISOString()}] SSR render request received:`,
      body,
    );
    ...
    const fontInfos = collectFontInfoFromOverlays(
      validatedData.inputProps.overlays || [],
    );
    ...
    console.log(
      `🎬 [${new Date().toISOString()}] SSR render started with ID: ${renderId} (took ${(endTime - startTime).toFixed(2)}ms)`,
    );

...
export async function GET(request: Request) {
  try {
    ...
    const externalApiUrl =
      ...
      "https://www.reactvideoeditor.com/api/templates";
    const apiKey = process.env.PROJECTS_API_KEY;

    if (!apiKey) {
      console.error(
        "[Projects API] PROJECTS_API_KEY environment variable is not set",
      );
    ...
    if (!response.ok) {
      ...
      console.error(
        `[Projects API] External API returned ${response.status}: ${errorText}`,
      );

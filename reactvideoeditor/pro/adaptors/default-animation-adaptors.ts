...
export const animationTemplates: Record<string, AnimationTemplate> = {
  ...
  pulse: {
    ...
    enter: (frame) => ({
      transform: `scale(${interpolate(frame, [0, 5, 10, 15], [0, 1.2, 0.9, 1], {
        extrapolateRight: "clamp",
      })})`,
      opacity: interpolate(frame, [0, 5], [0, 1], {
        extrapolateRight: "clamp",
  ...
  drop: {
    ...
    isPro: true,
    enter: (frame) => ({
      transform: `translateY(${interpolate(frame, [0, 10, 15], [-200, 20, 0], {
        extrapolateRight: "clamp",
      })}px)`,
 ...
 */
export const defaultAnimationAdaptor = createStaticAnimationAdaptor(
  animationTemplates,
  "Default Animations",
);

...
export const pexelsImageAdaptor: ImageAdaptor = {
  ...
  async search(
  ...
  ): Promise<MediaSearchResult<StandardImage>> {
    ...
    try {
      ...
      const standardImages: StandardImage[] = data.photos.map(
        transformPexelsImageToStandard,
      );
  ...
  getImageUrl(
    image: StandardImage,
    size: "original" | "large" | "medium" | "small" = "medium",
  ): string {
 ...
 */
function transformPexelsImageToStandard(
  pexelsImage: PexelsImageResponse,
): StandardImage {
  return {
    id: pexelsImage.id,
    type: "image",
 ...
 */
export const createPexelsImageAdaptor = (
  apiKey: string,
): ImageOverlayAdaptor => {
  return {
    ...pexelsImageAdaptor,

...
export const pexelsVideoAdaptor: VideoAdaptor = {
  ...
  async search(
  ...
  ): Promise<MediaSearchResult<StandardVideo>> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Pexels Video API error: ${response.status} ${response.statusText}`,
        );
      }
      ...
      const standardVideos: StandardVideo[] = data.videos.map(
        transformPexelsVideoToStandard,
      );
  ...
  getVideoUrl(
    video: StandardVideo,
    quality: "uhd" | "hd" | "sd" | "low" = "hd",
  ): string {
    const qualityFile = video.videoFiles.find(
 ...
 */
function transformPexelsVideoToStandard(
  pexelsVideo: PexelsVideoResponse,
): StandardVideo {
 ...
 */
export const createPexelsVideoAdaptor = (
  apiKey: string,
): VideoOverlayAdaptor => {
  return {
    ...pexelsVideoAdaptor,

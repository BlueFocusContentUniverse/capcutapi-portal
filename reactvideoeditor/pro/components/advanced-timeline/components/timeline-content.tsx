...
import { TimelineMarkersSection } from "./timeline-markers-section";
...
export const TimelineContent: React.FC<TimelineContentProps> = ({
  ...
  useEffect(() => {
    ...
    const handleGlobalTouchMove = (e: TouchEvent) => {
      ...
      if (isOutsideTimeline) {
        ...
        const clampedX = Math.max(
          timelineRect.left,
          Math.min(timelineRect.right, touch.clientX),
        );
        const clampedY = Math.max(
          timelineRect.top,
          Math.min(timelineRect.bottom, touch.clientY),
        );
        handleDrag(clampedX, clampedY);
      } else {
        // Normal drag handling when inside timeline
  ...
  useEffect(() => {
    ...
    const tracksScrollContainer = document.querySelector(
      ".timeline-tracks-scroll-container",
    );
    const markersOverlay = document.querySelector(
      ".timeline-markers-overlay-content",
    );
    ...
    const syncScroll = () => {
      const scrollLeft =
        (markersWrapper as HTMLElement).scrollLeft ||
        (tracksScrollContainer as HTMLElement).scrollLeft;
      (markersOverlay as HTMLElement).style.transform =
  ...
  return (
    <div
      ...
      style={
        {
          // Initialize CSS custom properties for ghost marker at root level so overlay can access them
    ...
    >
      ...
      <div
        ...
        style={{
          overflow:
            "scroll" /* Always show scrollbar to keep same height and width */,
        }}
      >
        <div
          ref={timelineRef}
        ...
        >
          <div className="timeline-tracks-container">
            {tracks.map((track, index) => {
              ...
              const trackGhostElements =
                ghostElement?.filter((ghost) => {
                  const calculatedIndex = Math.round(
                    (ghost.top * tracks.length) / 100,
                  );

...
export const TimelineGhostElement: React.FC<TimelineGhostElementProps> = ({
  ...
  if (isFloating) {
    return (
      <div
        className="fixed border border-(--border) rounded-[3px] pointer-events-none  z-9999 flex items-center justify-center"
        style={{
      ...
      >
        {itemData && (
          <div className="text-xs font-extralight text-white/90 text-center px-2 truncate"></div>
        )}
      </div>
    );
  }

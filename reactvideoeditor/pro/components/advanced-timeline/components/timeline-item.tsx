...
export const TimelineItem: React.FC<TimelineItemProps> = ({
  ...
  const [isShowingVideoThumbnails, setIsShowingVideoThumbnails] =
  ...
  const handleThumbnailDisplayChange = React.useCallback(
  ...
  const initiateDragStart = React.useCallback(
    (clientX: number, clientY: number, isTouch: boolean = false) => {
      ...
      const handlesVisible =
        !splittingEnabled && !isMultiSelected && !!onDragStart;
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (splittingEnabled && onSplitItems) {
      ...
      if (
        splitTime - item.start >= minSegmentDuration &&
        item.end - splitTime >= minSegmentDuration
      ) {
  ...
  const handleSplit = () => {
    ...
    if (
      leftSegmentDuration >= minSegmentDuration &&
      rightSegmentDuration >= minSegmentDuration
    ) {
    ...
    (e: React.MouseEvent) => {
      ...
      splitThrottleRef.current = requestAnimationFrame(() => {
        ...
        if (
          ...
          Math.abs(percentage - lastSplitPositionRef.current) > 0.5
        ) {
  ...
  const isPlayheadOverItem =
    currentFrame && fps
      ? currentFrame / fps >= item.start && currentFrame / fps <= item.end
      : false;
  const showSplitOption =
    !isMultiSelection && !!onSplitItems && isPlayheadOverItem;
  ...
  return (
    <ContextMenu onOpenChange={onContextMenuOpenChange}>
      <ContextMenuTrigger asChild>
        <div
          ref={itemRef}
          className={`timeline-item group absolute top-1/2 transform -translate-y-1/2 rounded flex items-center justify-center text-xs font-extralight text-white shadow-sm hover:shadow-md transition-shadow select-none overflow-hidden touch-none ${getCursorStyle().className} ${isSelected ? "border-2 border-(--timeline-item-selected-border)" : "border border-white/20"}`}
          style={{
            left: `${leftPercentage}%`,
            width: `${widthPercentage}%`,
            height: `${TIMELINE_CONSTANTS.TRACK_ITEM_HEIGHT}px`,
            backgroundColor: isShowingVideoThumbnails
              ? "transparent"
              : item.color || "#3b82f6",
        ...
        >
          ...
          {/* Debug label */}
          {/* <div className="absolute top-0 left-0 bg-black/80 text-white text-[8px] px-1 py-0.5 rounded-br z-10 pointer-events-none">
            S:{item.start.toFixed(2)} E:{item.end.toFixed(2)} D:{duration.toFixed(2)} MS:{item.mediaStart?.toFixed(2) || 'N/A'} ME:{item.mediaEnd?.toFixed(2) || 'N/A'} MD:{item.mediaStart !== undefined && item.mediaEnd !== undefined ? (item.mediaEnd - item.mediaStart).toFixed(2) : 'N/A'} MSD:{item.mediaSrcDuration?.toFixed(2) || 'N/A'} 
          </div>  */}

 ...
 */
export const TimelineItemContentFactory: React.FC<
  TimelineItemContentFactoryProps
> = ({
  type,
  label,
  data,
 ...
 */
const DefaultItemContent: React.FC<
  BaseItemContentProps & { type?: string }
> = ({ label, type, itemWidth }) => {
  return (
    <div className="flex items-center h-full w-full overflow-hidden px-2">
      ...
      <div className="flex-1 min-w-0">
        ...
        {itemWidth > 80 && type && (
          <div className="text-xs text-white/60 truncate">Type: {type}</div>
        )}
      </div>
    </div>
  );

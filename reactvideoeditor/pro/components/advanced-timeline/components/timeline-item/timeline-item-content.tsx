...
export const TimelineItemContent: React.FC<TimelineItemContentProps> = ({
    ...
    type === TrackItemType.AUDIO
      ? mediaStart !== undefined
        ...
        : data?.startFromSound !== undefined
          ? data.startFromSound / 30
          : 0 // Convert frames to seconds
      : 0;
  ...
  const thumbnailResult = useVideoThumbnailProcessor(
    type === TrackItemType.VIDEO && (data?.src || data?.originalUrl)
      ? data?.src || data?.originalUrl
      : undefined,
    videoContentStart,
    videoContentEnd,
    {
  ...
  const enhancedData =
    type === TrackItemType.VIDEO
      ? {
          ...audioEnhancedData,
          thumbnails:
            thumbnailResult.data?.thumbnails || audioEnhancedData?.thumbnails,
          thumbnailTimestamps:
            thumbnailResult.data?.timestamps ||
            audioEnhancedData?.thumbnailTimestamps,

...
export const TimelineItemResizeHandles: React.FC<
  TimelineItemResizeHandlesProps
> = ({
  onDragStart,
  ...
  const handleMouseDown =
    (position: "left" | "right") => (e: React.MouseEvent<HTMLDivElement>) => {
      onMouseDown?.(e, position);
    };

  const handleTouchStart =
    (position: "left" | "right") => (e: React.TouchEvent<HTMLDivElement>) => {
      ...
      >
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="space-x-0.5 flex ml-0">
            <div className="w-px h-4 bg-(--timeline-handle-indicator) rounded-full shadow-glow" />
            <div className="w-px h-4 bg-(--timeline-handle-indicator) rounded-full shadow-glow" />
          </div>
        </div>
      </div>

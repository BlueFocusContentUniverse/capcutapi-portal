    ...
    if (captions.length > 0) {
      return (
        <div className="flex items-center h-full w-full overflow-hidden px-2 transition-all duration-200 ease-out bg-caption-item">
          <div
            className={`flex items-center min-w-0 w-full overflow-hidden transition-all duration-200 ease-out ${
          ...
          >
            <div className="flex items-center p-1 rounded-sm overflow-hidden bg-caption-overlay/90">
              <div className="shrink-0 mr-2 flex items-center overflow-hidden">
                <LetterText className="w-4 h-4 shrink-0 text-caption-item-active-foreground" />
              </div>
              <div className="min-w-0 overflow-hidden">
                <div className="truncate text-xs font-extralight whitespace-nowrap text-caption-item-active-foreground">
  ...
  return (
    <div className="absolute inset-0 flex w-full h-full z-10 pointer-events-none bg-caption-item">
      {captions?.map((caption, index) => {
        const captionWidth =
          ((caption.endMs - caption.startMs) / (durationInSeconds * 1000)) *
          100;
          ...
          >
            {caption.words.map((word, wordIndex) => {
              ...
              return (
                <div
                  key={wordIndex}
                  className={`flex p-[3px] items-center justify-center mx-px rounded-[3px] transition-all duration-150 ${
                    isWordActive
                      ? "bg-caption-item-active "
                      : "bg-caption-item-foreground"
                  }`}
                  style={{
                    width: `${wordWidth}%`,
                  }}

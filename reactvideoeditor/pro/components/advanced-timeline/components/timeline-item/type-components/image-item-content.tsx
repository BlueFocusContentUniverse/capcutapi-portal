  ...
  if (imageSource && itemWidth > 60) {
    return (
      <div className="flex items-center h-full w-full overflow-hidden relative rounded-[3px]">
        ...
        <div className="h-full aspect-square shrink-0 mr-2">
          <div className="h-full w-full flex items-center py-2 relative">
            <img
              src={imageSource}
              alt=""
              draggable={false}
              onDragStart={(e) => e.preventDefault()}
              className="absolute inset-0 w-full h-full rounded-[1px] ml-6 object-cover"
            />
          </div>
        </div>

...
export const VideoItemContent: React.FC<VideoItemContentProps> = ({
  ...
  const isShowingThumbnails =
    thumbnails && thumbnails.length > 0 && !isLoadingThumbnails;
  ...
  if (isShowingThumbnails) {
    ...
    return (
      <div className="flex items-center h-full w-full overflow-hidden relative rounded-[3px]">
        ...
        {(isHovering || itemWidth < 80) &&
          showLabelOnHover && ( // Reduced from 120 to 80px to better accommodate higher thumbnail density

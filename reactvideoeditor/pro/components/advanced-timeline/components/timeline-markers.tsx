...
export const TimelineMarkers: React.FC<TimelineMarkersProps> = ({
        ...
        if (!hasMoved) {
          ...
          if (container) {
            ...
            if (timeInSeconds >= 0 && timeInSeconds <= totalDuration) {
              onTimeClick?.(
                Math.max(0, Math.min(totalDuration, timeInSeconds)),
              );
            }
          }
        }
  ...
  const handleTouchStart = useCallback(
    (e: React.TouchEvent<HTMLDivElement>) => {
      ...
      // Don't prevent default here - this causes the passive event listener error
      e.stopPropagation();
      ...
      // Add haptic feedback only if user has interacted with the page
      // Check for user activation to avoid the blocked vibrate call
      if ("vibrate" in navigator && navigator.userActivation?.hasBeenActive) {
        try {
          navigator.vibrate(10);
        } catch (error) {
          // Silently fail if vibrate is blocked
          console.debug("Vibrate blocked:", error);
        }
      }
      ...
      const handleTouchMove = (e: TouchEvent) => {
        // Only prevent default during move, not on start
        e.preventDefault();
        const touch = e.touches[0];
        if (!touch) return;
      ...
      // Use passive: false only for touchmove where we need preventDefault
    ...
    for (let time = 0; time <= totalDuration; time += interval) {
      ...
      markers.push(
        ...
        >
          ...
          <span className="text-xs mt-1 select-none whitespace-nowrap text-text-secondary font-extralight">

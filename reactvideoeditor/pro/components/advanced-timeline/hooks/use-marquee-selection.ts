    ...
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      if (timelineRef?.current) {
        ...
        if (!e.shiftKey) {
          ...
          console.log(
            "Marquee selection preserving current selection (Shift key held)",
          );
        }
      }
    },
        ...
        marqueeThrottleRef.current = requestAnimationFrame(() => {
          ...
          tracks.forEach((track, trackIndex) => {
            track.items.forEach((item) => {
              ...
              const xOverlaps =
                marqueeRect.x1 < itemRightPx && marqueeRect.x2 > itemLeftPx;
              const yOverlaps =
                marqueeRect.y1 < itemBottomPx && marqueeRect.y2 > itemTopPx;
          ...
          if (
            ...
            !Array.from(newlySelectedIds).every((id) =>

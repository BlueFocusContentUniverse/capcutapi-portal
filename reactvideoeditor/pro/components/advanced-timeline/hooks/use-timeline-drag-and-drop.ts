...
interface UseTimelineDragAndDropProps {
  ...
  onCreateTracksWithItems?: (
    ...
    trackItems: Array<{
      trackId: string;
      items: Array<{ itemId: string; start: number; end: number }>;
    }>,
  ) => void;
...
export const useTimelineDragAndDrop = ({
  ...
  const snapToGridAndEdges = useCallback(
    (value: number, trackIndex: number, excludeIds: string[] = []) => {
      // First, get basic grid snap
      const gridSnapped =
        Math.round(value / SNAPPING_CONFIG.gridSize) * SNAPPING_CONFIG.gridSize;

      // Skip edge snapping for magnetic tracks or if no tracks available
      if (
        trackIndex < 0 ||
        trackIndex >= tracks.length ||
        tracks[trackIndex].magnetic
      ) {
        return gridSnapped;
      }
  ...
  const validateDropPosition = useCallback(
    (
      ...
      if (targetStart < 0) {
        return {
          isValid: false,
          reason: "Cannot place item before timeline start",
        };
      }
      ...
      if (action === "resize-start" || action === "resize-end") {
        ...
        if (dragInfo && dragInfo.selectedItemsSnapshot.length > 0) {
          const primaryItem = dragInfo.selectedItemsSnapshot.find(
            (item) => item.id === dragInfo.id,
          );
          if (
            primaryItem &&
            (primaryItem.type === "video" || primaryItem.type === "audio") &&
            primaryItem.mediaSrcDuration !== undefined
          ) {
            ...
            if (action === "resize-start") {
              ...
              // When start moves later (positive delta), we need to skip more media content at the beginning
              // When start moves earlier (negative delta), we need to show more media content from the beginning
              calculatedMediaStart = Math.max(
                0,
                calculatedMediaStart + startTimeDelta,
              );

              // Check if we're trying to resize before the beginning of media content
              if (
                startTimeDelta < 0 &&
                (primaryItem.mediaStart || 0) + startTimeDelta < 0
              ) {
                return {
                  isValid: false,
                  reason: `Cannot resize before the beginning of media content`,
                };
              }
            }

            const maxAllowedDuration =
              primaryItem.mediaSrcDuration - calculatedMediaStart;
            ...
            if (targetDuration > maxAllowedDuration + DURATION_TOLERANCE) {
              return {
                isValid: false,
                reason: `Cannot resize beyond source duration (${maxAllowedDuration.toFixed(1)}s available)`,
              };
            }
          }
        }
      ...
      if (targetTrack.magnetic) {
        const dragInfo = getDragInfo();
        const isMultiDrag =
          dragInfo && dragInfo.selectedItemsSnapshot.length > 1;
        ...
        // Only apply magnetic insertion logic for move operations
        if (action === "move") {
          const availableItems = targetTrack.items.filter(
        ...
        // For resize on magnetic tracks, allow without altering start via insertion logic
        return { isValid: true };
      }
  ...
  const handleDragStart = useCallback(
    (
      ...
      const itemTrackIndex = tracks.findIndex(
      ...
      for (const itemId of itemsToDrag) {
        ...
        if (draggedItem) {
          const draggedItemTrackIndex = tracks.findIndex(
            (track) => track.id === draggedItem.trackId,
          );
      ...
      const primarySnapshot = selectedItemsSnapshot.find(
        (s) => s.id === item.id,
      );
  ...
  const handleDrag = useCallback(
    (clientX: number, clientY: number) => {
      ...
      if (isMultiDrag) {
        for (const snapshot of dragInfo.selectedItemsSnapshot) {
          ...
          if (itemNewTrackIndex >= tracks.length) {
            ...
            constrainedDeltaTrack = Math.min(
              constrainedDeltaTrack,
              maxAllowedDelta,
            );
          }
          ...
          if (itemNewTrackIndex < 0) {
            ...
            constrainedDeltaTrack = Math.max(
              constrainedDeltaTrack,
              minAllowedDelta,
            );
          }
      ...
      const effectiveDeltaTrack = isMultiDrag
        ? constrainedDeltaTrack
        : deltaTrack;
      const targetTrackIndex = Math.max(
        0,
        Math.min(tracks.length - 1, dragInfo.startRow + effectiveDeltaTrack),
      );
      ...
      switch (dragInfo.action) {
        ...
        case "resize-start":
          ...
          const snappedNewStart = snapToGridAndEdges(
            rawNewStart,
            dragInfo.startRow,
            excludeIds,
          );
          ...
          if (
            ...
            // When start moves later (positive delta), we need to skip more media content at the beginning
            // When start moves earlier (negative delta), we need to show more media content from the beginning
            const calculatedMediaStart = Math.max(
              0,
              (primaryItem.mediaStart || 0) + startTimeDelta,
            );

            // Prevent resizing before the beginning of media content
            if (startTimeDelta < 0 && calculatedMediaStart <= 0) {
              // Can't resize earlier than the available media content
              const maxEarlierDelta = -(primaryItem.mediaStart || 0);
              newStart = primaryItem.originalStart + maxEarlierDelta;
        ...
        case "resize-end":
          ...
          newDuration = Math.max(
            MIN_ITEM_DURATION,
            snappedEndPosition - newStart,
          );
          newTrackIndex = dragInfo.startRow; // Don't change track during resize

          // Apply source duration validation for video/audio items
          const primaryItemEnd = dragInfo.selectedItemsSnapshot.find(
            (item) => item.id === dragInfo.id,
          );
          if (
            primaryItemEnd &&
            (primaryItemEnd.type === "video" ||
              primaryItemEnd.type === "audio") &&
            primaryItemEnd.mediaSrcDuration !== undefined
          ) {
            const mediaStartOffset = primaryItemEnd.mediaStart || 0;
            const maxAllowedDuration =
              primaryItemEnd.mediaSrcDuration - mediaStartOffset;
      ...
      if (newStart + newDuration > totalDuration) {
        if (dragInfo.action === "move") {
          newStart = Math.max(0, totalDuration - newDuration);
        } else {
          newDuration = Math.max(MIN_ITEM_DURATION, totalDuration - newStart);
        }
      }
      ...
      if (isMultiDrag) {
        ...
        const primarySnapshot = dragInfo.selectedItemsSnapshot.find(
          (s) => s.id === dragInfo.id,
        );
        if (!primarySnapshot) return;
        ...
        for (const snapshot of dragInfo.selectedItemsSnapshot) {
          if (snapshot.id !== dragInfo.id) {
            ...
            const itemDuration = snapshot.originalDuration;

            // Check left boundary constraint
            if (itemNewStart < 0) {
              const leftConstraint = -snapshot.originalStart;
              constrainedDeltaTime = Math.max(
            ...
            if (itemNewStart + itemDuration > totalDuration) {
              const rightConstraint =
                totalDuration - itemDuration - snapshot.originalStart;
              constrainedDeltaTime = Math.min(
                constrainedDeltaTime,
                rightConstraint,
              );
            }
        ...
        for (const snapshot of dragInfo.selectedItemsSnapshot) {
          ...
          if (snapshot.id === dragInfo.id) {
            // Primary item uses constrained values
            itemNewStart = Math.max(
              0,
              Math.min(
                totalDuration - finalDuration,
                snapshot.originalStart + deltaTime,
              ),
            );
            itemNewDuration = finalDuration;
            itemNewTrackIndex = Math.max(
              0,
              Math.min(
                tracks.length - 1,
                snapshot.originalRow + finalDeltaTrack,
              ),
            );
          } else {
            // Other items maintain relative position to primary using constrained deltas
            itemNewStart = Math.max(
              0,
              Math.min(
                totalDuration - snapshot.originalDuration,
                snapshot.originalStart + deltaTime,
              ),
            );
          ...
          const itemGhost = calculateGhostPosition(
            itemNewStart,
            itemNewDuration,
            itemNewTrackIndex,
          );
        ...
        // Single item drag
        const ghostElement = calculateGhostPosition(
          finalStart,
          finalDuration,
          newTrackIndex,
        );
      ...
      const contentScrollTop =
        (timelineRef.current as HTMLDivElement).scrollTop || 0;
      ...
      if (distanceToBoundary < SHOW_THRESHOLD && dragInfo.action === "move") {
        ...
        const insertionIdx = Math.max(
          0,
          Math.min(tracks.length, nearestBoundary),
        );
  ...
  const handleDragEnd = useCallback(() => {
    ...
    if (
      ...
      if (isMultiDrag && onCreateTracksWithItems) {
        ...
        if (primarySnapshot) {
          const primaryFinalStart = snapToGrid(
            dragInfo.currentStart ?? dragInfo.startPosition,
          );
          ...
          for (const snapshot of dragInfo.selectedItemsSnapshot) {
            const trackItems =
              itemsByOriginalTrack.get(snapshot.originalRow) || [];
          ...
          const sortedTrackGroups = Array.from(
            itemsByOriginalTrack.entries(),
          ).sort(([a], [b]) => a - b);
          const trackItemsData = sortedTrackGroups.map(
            ([originalTrackIndex, trackItems]) => {
              ...
              const items = trackItems.map((snapshot) => ({
                ...
                start: Math.max(0, snapshot.originalStart + deltaTime),
                  ...
                  Math.max(0, snapshot.originalStart + deltaTime) +
                  snapshot.originalDuration,
              }));
        ...
        const finalDuration = snapToGrid(
          dragInfo.currentDuration ?? dragInfo.startDuration,
        );
    ...
    if (isMultiDrag && dragInfo.action === "move" && onItemMove) {
      ...
      for (let i = 0; i < dragInfo.selectedItemsSnapshot.length; i++) {
        ...
        if (ghostElement) {
          const finalStart = snapToGrid(
            (ghostElement.left / 100) * totalDuration,
          );
          const finalDuration = snapToGrid(
            (ghostElement.width / 100) * totalDuration,
          );
          const finalTrackIndex = Math.round(
            (ghostElement.top * tracks.length) / 100,
          );
          const finalEnd = finalStart + finalDuration;

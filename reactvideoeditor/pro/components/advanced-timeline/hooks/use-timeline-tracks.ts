...
export const useTimelineTracks = ({
  ...
  const [isAutoRemoveEnabled, setIsAutoRemoveEnabled] = useState<boolean>(
    autoRemoveEmptyTracks,
  );
  ...
  const removeEmptyTracks = useCallback(
    (
      tracks: TimelineTrack[],
      shouldRemove: boolean = isAutoRemoveEnabled,
    ): TimelineTrack[] => {
            ...
            if (track.id === newItem.trackId) {
              ...
              if (track.magnetic) {
                ...
                for (let i = 0; i < preview.insertionIndex; i++) {
                  ...
                  const originalItem = track.items.find(
                ...
                for (
                  let i = preview.insertionIndex;
                  i < preview.previewItems.length;
                  i++
                ) {
                  const previewItem = preview.previewItems[i];
                  const originalItem = track.items.find(
                ...
                updatedTrack.items = [...track.items, newItem].sort(
                  (a, b) => a.start - b.start,
                );
              }
  ...
  const handleItemMove = useCallback(
    (itemId: string, newStart: number, newEnd: number, newTrackId: string) => {
      setTracks((prevTracks) => {
        ...
        if (targetTrackIndex !== -1) {
          ...
          if (movedItem) {
            ...
            if (targetTrack.magnetic) {
              ...
              const itemsForInsertion = targetTrack.items.filter(
                (item) => item.id !== itemId,
              );
              ...
              for (let i = 0; i < preview.insertionIndex; i++) {
                const previewItem = preview.previewItems[i];
                const originalItem = itemsForInsertion.find(
                  (item) => item.id === previewItem.id,
                );
                if (originalItem) {
                  magneticItems.push({
        ...
        if (sourceTrack && sourceTrack.magnetic) {
          ...
          if (sourceTrackIndex !== -1) {
            const closedGapItems = closeGapsInTrack(
              newTracks[sourceTrackIndex].items,
            );
  ...
  const handleItemResize = useCallback(
    (itemId: string, newStart: number, newEnd: number) => {
      setTracks((prevTracks) => {
                ...
                if (item.id === itemId) {
                  ...
                  if (
                    ...
                    if (startTimeDelta !== 0) {
                      ...
                      updatedMediaStart = Math.max(
                        0,
                        item.mediaStart + startTimeDelta,
                      );
                    }
                  }
                  ...
                  if (
                    ...
                    if (
                      proposedDuration >
                      maxAllowedDuration + DURATION_TOLERANCE
                    ) {
  ...
  const handleItemsDelete = useCallback(
    (itemIds: string[]) => {
      setTracks((prevTracks) => {
        const newTracks = prevTracks.map((track) => {
          const sourceTrack = track.items.some((item) =>
  ...
  const handleInsertTrackAt = useCallback(
    (index: number): string => {
      let newId = "";
      setTracks((prev) => {
          ...
          items: track.items.filter(
            (item) =>
              !trackItems.some((trackGroup) =>
                trackGroup.items.some(
                  (itemInfo) => itemInfo.itemId === item.id,
                ),
              ),
          ),
        }));
  ...
  const clearSelectionForDeletedTrack = useCallback(
    (trackToDelete: TimelineTrack) => {
      if (
        !trackToDelete ||
      ...
      if (selectedItemsInDeletedTrack.length > 0) {
        const remainingSelectedItems = selectedItemIds.filter(
          (itemId) => !itemsInDeletedTrack.includes(itemId),
        );

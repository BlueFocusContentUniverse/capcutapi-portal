...
class ThumbnailCache {
  ...
  private createKey(src: string | File, start: number, end: number): string {
    const sourceId =
      src instanceof File
    ...
    const generateThumbnails = async () => {
      try {
        ...
        const width =
          aspectRatio >= 1
            ? thumbnailSize
            : Math.floor(thumbnailSize * aspectRatio);
        const height =
          aspectRatio <= 1
            ? thumbnailSize
            : Math.floor(thumbnailSize / aspectRatio);
        ...
        const actualEnd =
            ...
            ? Math.min(end, firstTimestamp + videoDuration)
            : firstTimestamp + videoDuration;
        ...
        for (let i = 0; i < thumbnailCount; i++) {
          const timestamp =
            actualStart + (i * rangeDuration) / Math.max(thumbnailCount - 1, 1);

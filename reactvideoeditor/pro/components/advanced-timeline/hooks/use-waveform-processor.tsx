  ...
  useEffect(() => {
    ...
    const generateWaveform = async () => {
      ...
      try {
        ...
        const clampedStartSample = Math.max(
          0,
          Math.min(startSample, channelData.length),
        );
        const clampedEndSample = Math.max(
          clampedStartSample,
        ...
        const samplesPerPeak = Math.max(
          1,
          Math.floor((clampedEndSample - clampedStartSample) / targetPeaks),
        );

        const peaks: number[] = [];

        for (
          let i = clampedStartSample;
          i < clampedEndSample;
          i += samplesPerPeak
        ) {
          let peak = 0;
        ...
        if (error instanceof Error && error.name === "AbortError") {
          console.debug("Waveform processing aborted (expected behavior):", {
            src,
            startTime,
            duration,
          });

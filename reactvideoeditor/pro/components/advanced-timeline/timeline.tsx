...
export const Timeline = forwardRef<TimelineRef, TimelineProps>(
  (
    ...
    const { zoomScale, setZoomScale, handleWheelZoom } =
    ...
    const canUndo =
      parentCanUndo !== undefined ? parentCanUndo : internalCanUndo;
    const canRedo =
      parentCanRedo !== undefined ? parentCanRedo : internalCanRedo;
    ...
    const { compositionDuration, viewportDuration, currentTime } =
      useTimelineComposition({
        tracks,
        totalDuration,
        currentFrame,
    ...
    const handleCombinedItemMove = useCallback(
      (
        itemId: string,
        newStart: number,
        newEnd: number,
    ...
    const isPlayheadOverSelectedItem = useCallback(() => {
      ...
      for (const track of tracks) {
        ...
        if (item) {
          ...
          return (
            currentTimeInSeconds >= item.start &&
            currentTimeInSeconds <= item.end
          );
        }
      }
      return false;
    ...
    const handleSplitAtSelection = useCallback(() => {
      ...
      // Check if the current playhead is within the item's time range
      if (
        currentTimeInSeconds < selectedItem.start ||
        currentTimeInSeconds > selectedItem.end
      ) {
        console.warn(
          "Current playhead is not within the selected item's time range",
        );
        return;
      }
    ...
    const handleCombinedItemResize = useCallback(
    ...
    const handleCombinedItemsDelete = useCallback(
      (itemIds: string[]) => {
    ...
    const handleCombinedAddNewItem = useCallback(
      (itemData: {
        type: string;
        label?: string;
        duration?: number;
    ...
    const handleEnhancedToggleAutoRemoveEmptyTracks = useCallback(
      (enabled: boolean) => {
    ...
    return (
      <div className="timeline-container bg-background flex flex-col h-full overflow-hidden">
        <TimelineHeader
          ...
          autoRemoveEmptyTracks={isAutoRemoveEnabled}
          onToggleAutoRemoveEmptyTracks={
            handleEnhancedToggleAutoRemoveEmptyTracks
          }
          splittingEnabled={isSplittingEnabled}
          onToggleSplitting={handleToggleSplitting}
          onSplitAtSelection={handleSplitAtSelection}
          hasSelectedItem={
            selectedItemIds.length === 1 && isPlayheadOverSelectedItem()
          }

...
export const pushItemsDuringResize = (
  ...
  if (expandingLeft) {
    ...
    const blockingItems = trackItems.filter(
      (item) =>
  ...
  const updatedItems = trackItems.map((item) => {
    if (item.id === resizedItemId) {
      ...
      if (
        ...
        const maxAllowedDuration = item.mediaSrcDuration - mediaStartOffset;
  ...
  if (expandingRight) {
    ...
    const resizedNewItem = updatedItems.find(
      (item) => item.id === resizedItemId,
    )!;
    ...
    for (let i = 0; i < updatedItems.length; i++) {
      ...
      if (
        currentItem.start < resizedNewItem.end &&
        currentItem.end > resizedNewItem.start
      ) {

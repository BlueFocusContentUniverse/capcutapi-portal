...
export const useTimelineHandlers = ({
  ...
  const handleTracksChange = React.useCallback(
  ...
  const handleTimelineFrameChange = React.useCallback(
    (frame: number) => {
  ...
  const setSidebarForOverlay = React.useCallback(
    (overlayId: number) => {
  ...
  const handleDuplicateItems = React.useCallback(
    (itemIds: string[]) => {
      itemIds.forEach((itemId) => {
        const overlayId = parseInt(itemId, 10);
  ...
  const handleSplitItems = React.useCallback(
    (itemId: string, splitTime: number) => {
      const overlayId = parseInt(itemId, 10);
  ...
  const handleItemMove = React.useCallback(
    (itemId: string, newStart: number, newEnd: number, newTrackId: string) => {
      ...
      console.log(`[TIMELINE-HANDLERS][${timestamp}] handleItemMove CALLED:`, {
        ...
        currentOverlayState: overlays.map((o) => ({
          id: o.id,
          row: o.row,
          type: o.type,
        })),
      ...
      if (overlay) {
        ...
        console.error(
          `[TIMELINE-HANDLERS][${timestamp}] ❌ Overlay not found for move:`,
          { itemId, overlayId, availableOverlays: overlays.map((o) => o.id) },
        );
      }
    },
  ...
  const handleItemResize = React.useCallback(
    (itemId: string, newStart: number, newEnd: number) => {
      ...
      if (isUpdatingFromTimelineRef.current) {
        console.log(
          `[TIMELINE-HANDLERS][${timestamp}] 🛑 SKIPPING handleItemResize - batch update in progress`,
        );
        return;
      }
  ...
  const handleNewItemDrop = React.useCallback(
    (
      ...
      console.log("[TIMELINE-HANDLERS] New item drop", {
        itemType,
        trackIndex,
        startTime,
        itemData,
      ...
      try {
        ...
        if (itemType === "video" && itemData?.data) {
          ...
          newOverlay = {
            ...
            styles: {
              ...
              zIndex: 1,
              transform: "none",
              textAlign: template.styles.textAlign as
                | "left"
                | "center"
                | "right",
        ...
        // Generate new ID
        const newId =
          overlays.length > 0 ? Math.max(...overlays.map((o) => o.id)) + 1 : 0;
        ...
        console.log(
          "[TIMELINE-HANDLERS] Successfully created overlay",
          overlayWithId,
        );
      } catch (error) {
        console.error(
          "[TIMELINE-HANDLERS] Error creating overlay from drop:",
          error,
        );
      }
    },

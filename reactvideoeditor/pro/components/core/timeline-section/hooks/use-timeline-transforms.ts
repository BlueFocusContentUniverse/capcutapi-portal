...
export const useTimelineTransforms = () => {
   ...
   */
  const transformOverlaysToTracks = React.useCallback(
    (overlays: Overlay[]): TimelineTrack[] => {
        ...
        const items: TimelineItem[] = overlaysInRow.map((overlay) => {
          ...
          if (overlay.type === OverlayType.VIDEO) {
            ...
            const videoStartTimeSeconds =
              typeof videoOverlay.videoStartTime === "number"
                ? videoOverlay.videoStartTime
                : 0;
            ...
            return {
              ...
              ...(videoOverlay.mediaSrcDuration && {
                ...
                mediaEnd:
                  videoStartTimeSeconds + overlay.durationInFrames / FPS,
              }),
            };
          }
          ...
          if (overlay.type === OverlayType.SOUND) {
            ...
            const audioStartTimeSeconds =
              typeof audioOverlay.startFromSound === "number"
                ? audioOverlay.startFromSound / FPS
                : 0;
   ...
   */
  const transformTracksToOverlays = React.useCallback(
    (tracks: TimelineTrack[]): Overlay[] => {
      ...
      tracks.forEach((track, trackIndex) => {
        track.items.forEach((item) => {
          if (item.data && typeof item.data === "object") {
            ...
            if (
              originalOverlay.type === OverlayType.VIDEO &&
              ...
              originalOverlay.type === OverlayType.SOUND &&
              item.mediaStart !== undefined
            ) {
              // Convert mediaStart from seconds back to frames for startFromSound
              (updatedOverlay as any).startFromSound = Math.round(
                item.mediaStart * FPS,
              );
            }

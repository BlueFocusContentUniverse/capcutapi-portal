...
import {
  formatBytes,
  formatDuration,
} from "../../../utils/general/format-utils";
...
} from "../../advanced-timeline/hooks/use-new-item-drag";
...
export function LocalMediaGallery({
    ...
    () => [
      {
        ...
        itemCount: localMediaFiles.filter((file) => file.type === "image")
          .length,
      },
      {
        adaptorName: "video",
        adaptorDisplayName: "Videos",
        itemCount: localMediaFiles.filter((file) => file.type === "video")
          .length,
      },
      {
        adaptorName: "audio",
        adaptorDisplayName: "Audio",
        itemCount: localMediaFiles.filter((file) => file.type === "audio")
          .length,
      },
    ],
    [localMediaFiles],
  ...
  const handleDragStart = useCallback(
    (file: any) => (e: React.DragEvent) => {
      // Extract duration from file if available
      const fileDuration = file.duration;
      const defaultDuration =
        file.type === "video" ? 5 : file.type === "audio" ? 5 : 5; // Default to 5 seconds
      const duration =
      ...
      if (file.path.startsWith("blob:")) {
        ...
        const apiPath = file.path.startsWith("/")
  ...
  const renderPreviewContent = () => {
    ...
    switch (selectedFile.type) {
      ...
      default:
        return (
          <div className="text-sm text-secondary">Unsupported file type</div>
        );
    }
  };
          ...
          <div className="aspect-video relative">
            {file.type === "image" && (
              <img
                src={
                  file.thumbnail ||
                  `/api/latest/local-media/serve/${file.path.startsWith("/") ? file.path.substring(1) : file.path}`
                }
                alt={file.name}

...
export const SoundDetails: React.FC<SoundDetailsProps> = ({
  ...
  const [duration, setDuration] = useState<number>(
    localOverlay.mediaSrcDuration ?? 10,
  );
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4 rounded-md bg-card p-4 border">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">
                {(localOverlay?.styles?.fadeIn ?? 0).toFixed(1)}s /{" "}
            ...
            <div className="flex items-center gap-3 pt-1">
              <Slider
                value={[
                  Math.max(
                    0,
                    Math.min(localOverlay?.styles?.fadeIn ?? 0, duration),
                  ),
                ]}
          ...
          <div className="space-y-4 rounded-md bg-card p-4 border">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">
                {(localOverlay?.styles?.fadeOut ?? 0).toFixed(1)}s /{" "}
                {duration.toFixed(1)}s
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 pt-1">
              <Slider
                value={[
                  Math.max(
                    0,
                    Math.min(localOverlay?.styles?.fadeOut ?? 0, duration),
                  ),
                ]}

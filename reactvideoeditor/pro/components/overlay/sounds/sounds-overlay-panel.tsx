...
const SoundsOverlayPanel: React.FC = () => {
  ...
  const handleAddToTimeline = async (sound: AudioWithSource) => {
    ...
    if (!sound.file || sound.file.trim() === "") {
      console.error(
        "Cannot add sound to timeline: No URL provided for sound",
        sound.title,
      );
    ...
    <div className="flex flex-col p-2 bg-background h-full overflow-hidden">
      {!localOverlay ? (
        <>
          {/* Source Tabs */}
          {searchResults &&
            searchResults.sourceResults &&
            searchResults.sourceResults.length > 0 && (
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-8 text-muted-foreground text-center">
                ...
                <p>
                  ...
                  {activeTab !== "all"
                    ? ` in ${searchResults.sourceResults.find((s: any) => s.adaptorName === activeTab)?.adaptorDisplayName}`
                    : ""}
                </p>
              </div>
            ) : (

...
export const TextStylePanel: React.FC<TextStylePanelProps> = ({
  ...
  const { loadFontForPreview, makeFontPreviewName, isFontLoaded } =
    ...
    const fetchFontWeights = async () => {
      try {
        const response = await fetch(
          `/api/fonts/${localOverlay.styles.fontFamily}`,
        );

        if (!response.ok) {
          console.warn(
            `Failed to fetch font metadata for ${localOverlay.styles.fontFamily}`,
          );
    ...
    <TooltipProvider>
      <div className="space-y-2">
        ...
        <div className="space-y-4 rounded-md bg-sidebar p-2.5 border">
                    ...
                    {filteredFonts.length === 0 ? (
                      ...
                      filteredFonts.map((font) => (
                        <FontItem
                          ...
                          isSelected={
                            localOverlay.styles.fontFamily === font.fontFamily
                          }
                          onClick={() => {
          ...
          <div className="space-y-4">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        type="button"
                        onClick={() =>
                          handleStyleChange("fontSizeScale", undefined)
                        }
            ...
            <Slider
              ...
              onValueChange={(value) =>
                handleStyleChange("fontSizeScale", value[0])
              }
              min={0.3}
              max={3}
              step={0.1}
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        type="button"
                        onClick={() =>
                          handleStyleChange("letterSpacing", undefined)
                        }
                        className="text-xs px-2 py-1.5 rounded-md transition-colors bg-secondary hover:bg-secondary/80 text-secondary-foreground"
                      >
                        <RotateCcw className="h-3 w-3" />
        ...
        <div className="space-y-2 rounded-md bg-sidebar p-2.5 border">
          <h5 className="text-sm font-light leading-none text-foreground">
            Colors
          </h5>

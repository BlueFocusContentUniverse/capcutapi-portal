 ...
 */
export const VideoAIPanel: React.FC<VideoAIPanelProps> = ({ localOverlay }) => {
  const {
    progress,
    error,
    isProcessing,
  ...
  const [greenscreenEnabled, setGreenscreenEnabled] = useState(
    localOverlay.greenscreen?.enabled ?? false,
  );
  const [greenscreenSensitivity, setGreenscreenSensitivity] = useState(
    localOverlay.greenscreen?.sensitivity ?? 100,
  );
  const [redThreshold, setRedThreshold] = useState(
    localOverlay.greenscreen?.threshold?.red ?? 100,
  );
  const [greenMin, setGreenMin] = useState(
    localOverlay.greenscreen?.threshold?.green ?? 100,
  );
  const [blueThreshold, setBlueThreshold] = useState(
    localOverlay.greenscreen?.threshold?.blue ?? 100,
  );
  const [smoothing, setSmoothing] = useState(
    localOverlay.greenscreen?.smoothing ?? 0,
  );
            ...
            <div className="space-y-3">
              ...
              {error && (
                <div className="p-2 rounded bg-destructive/10 border border-destructive/20">
                  <p className="text-xs text-destructive font-extralight">
                    {error}
                  </p>
                </div>
              )}

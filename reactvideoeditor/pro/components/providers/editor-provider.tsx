...
export const EditorProvider: React.FC<EditorProviderProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (hasProjectId) {
        console.log(
          "[EditorProvider] Project loaded, applying project overlays:",
  ...
  const [timelineItemHeight, setTimelineItemHeight] = useState(
    TIMELINE_CONSTANTS.TRACK_ITEM_HEIGHT,
  );
  const { durationInFrames, durationInSeconds } = useCompositionDuration(
    overlays,
    fps,
  );
  ...
  const {
    aspectRatio,
    setAspectRatio: setAspectRatioBase,
    playerDimensions,
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (hasProjectId) {
        console.log(
          "[EditorProvider] Project loaded, applying project aspect ratio:",
          defaultAspectRatio,
        );
  ...
  const { width: dynamicWidth, height: dynamicHeight } =
    getAspectRatioDimensions();
  ...
  const deleteOverlaysByRow = useCallback(
    (row: number) => {
      const overlaysToDelete = overlays.filter(

...
export const CropOverlay: React.FC<{
  ...
  const offsetX = isUsingEffectiveDimensions
    ? effectiveDimensions.left - overlay.left
    : 0;
  const offsetY = isUsingEffectiveDimensions
    ? effectiveDimensions.top - overlay.top
    : 0;
      ...
      return (e: React.MouseEvent) => {
        ...
        const onPointerMove = (pointerMoveEvent: PointerEvent) => {
          ...
          if (type === "left") {
            ...
            const newCropLeftPx = Math.max(
              0,
              Math.min(cropRightPx - 10, cropLeftPx + offsetX),
            );
          ...
          if (type === "right") {
            ...
            const newCropRightPx = Math.max(
              cropLeftPx + 10,
              Math.min(overlay.width, cropRightPx + offsetX),
            );
            const newCropWidth =
              ((newCropRightPx - cropLeftPx) / overlay.width) * 100;
            updates.cropWidth = Math.round(newCropWidth);
          }

          if (type === "top") {
            // Moving top edge - affects cropY and cropHeight
            const newCropTopPx = Math.max(
              0,
              Math.min(cropBottomPx - 10, cropTopPx + offsetY),
            );
          ...
          if (type === "bottom") {
            ...
            const newCropBottomPx = Math.max(
              cropTopPx + 10,
              Math.min(overlay.height, cropBottomPx + offsetY),
            );
            const newCropHeight =
              ((newCropBottomPx - cropTopPx) / overlay.height) * 100;
            updates.cropHeight = Math.round(newCropHeight);
          }

          // Generate new clipPath based on updated values
          const finalCropX =
            updates.cropX !== undefined ? updates.cropX : cropX;
          const finalCropY =
            updates.cropY !== undefined ? updates.cropY : cropY;
          const finalCropWidth =
            updates.cropWidth !== undefined ? updates.cropWidth : cropWidth;
          const finalCropHeight =
            updates.cropHeight !== undefined ? updates.cropHeight : cropHeight;

          updates.clipPath = generateClipPath(
            finalCropX,
            finalCropY,
            finalCropWidth,
            finalCropHeight,

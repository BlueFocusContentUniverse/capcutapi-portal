  ...
  const onPointerDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onPointerMove = (pointerMoveEvent: PointerEvent) => {
        ...
        const intendedNewEffectiveWidth =
          effectiveDimensions.width + (isLeft ? -offsetX : offsetX);
        const intendedNewEffectiveHeight =
          effectiveDimensions.height + (isTop ? -offsetY : offsetY);
        const intendedNewEffectiveLeft =
          effectiveDimensions.left + (isLeft ? offsetX : 0);
        const intendedNewEffectiveTop =
          effectiveDimensions.top + (isTop ? offsetY : 0);
        ...
        const intendedNewLeft =
          intendedNewEffectiveLeft - effectiveOffsetX * scaleX;
        const intendedNewTop =
          intendedNewEffectiveTop - effectiveOffsetY * scaleY;
        ...
        setOverlay(overlay.id, (i) => {
          ...
          if (shouldMaintainAspectRatio) {
            ...
            finalHeight = Math.max(
              1,
              Math.round(overlay.height * uniformScale),
            );

            // Recalculate position based on uniform scale
            const newEffectiveWidth = effectiveDimensions.width * uniformScale;
            const newEffectiveHeight =
              effectiveDimensions.height * uniformScale;

            // Adjust position based on which corner is being dragged
            if (type === "top-left") {
              finalLeft = Math.round(
                effectiveDimensions.left +
                  effectiveDimensions.width -
                  newEffectiveWidth -
                  effectiveOffsetX * uniformScale,
              );
              finalTop = Math.round(
                effectiveDimensions.top +
                  effectiveDimensions.height -
                  newEffectiveHeight -
                  effectiveOffsetY * uniformScale,
              );
            } else if (type === "top-right") {
              finalLeft = Math.round(
                effectiveDimensions.left - effectiveOffsetX * uniformScale,
              );
              finalTop = Math.round(
                effectiveDimensions.top +
                  effectiveDimensions.height -
              ...
              finalTop = Math.round(
                effectiveDimensions.top - effectiveOffsetY * uniformScale,
              );
            } else {
              // bottom-right
              finalLeft = Math.round(

...
export const SelectionOutline: React.FC<{
  ...
  const style: React.CSSProperties = useMemo(() => {
    ...
    return {
      width: Number.isFinite(effectiveDimensions.width)
        ? effectiveDimensions.width
        : 0,
      height: Number.isFinite(effectiveDimensions.height)
  ...
  const startDragging = useCallback(
    (e: PointerEvent | React.MouseEvent) => {
      ...
      const onPointerMove = (pointerMoveEvent: PointerEvent) => {
        ...
        const snapPosition = alignmentGuides.calculateSnapPosition(
          tempOverlay,
          allOverlays,
        );

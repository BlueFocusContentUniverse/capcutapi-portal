...
export const MediaAdaptorProvider: React.FC<MediaAdaptorProviderProps> = ({
  ...
  const searchVideos = useCallback(
    async (
      ...
      const mergedItems: Array<
        StandardVideo & { _source: string; _sourceDisplayName: string }
      > = [];
  ...
  const searchImages = useCallback(
    async (
      ...
      // Merge and attribute results
      const mergedItems: Array<
        StandardImage & { _source: string; _sourceDisplayName: string }
      > = [];
  ...
  const searchAudio = useCallback(
    async (
      ...
      // Merge and attribute results
      const mergedItems: Array<
        StandardAudio & { _source: string; _sourceDisplayName: string }
      > = [];
  ...
  const getTextTemplates = useCallback(
    async (
      config?: Record<string, any>,
    ): Promise<MergedSearchResult<TextOverlayTemplate>> => {
      ...
      const templatePromises = activeAdaptors.map(async (adaptor) => {
        try {
          ...
          console.error(
            `Error fetching text templates from ${adaptor.name}:`,
            error,
          );
          return {
            adaptor,
      ...
      // Merge and attribute results
      const mergedItems: Array<
        TextOverlayTemplate & { _source: string; _sourceDisplayName: string }
      > = [];
  ...
  const getStickerTemplates = useCallback(async (): Promise<
    MergedSearchResult<StickerTemplateConfig>
  > => {
  ...
  const getAnimationTemplates = useCallback(
    async (
      config?: Record<string, any>,
    ): Promise<MergedSearchResult<AnimationTemplate>> => {
      ...
      const templatePromises = activeAdaptors.map(async (adaptor) => {
        try {
          ...
          console.error(
            `Error fetching animation templates from ${adaptor.name}:`,
            error,
          );
          return {
            adaptor,
      ...
      // Merge and attribute results
      const mergedItems: Array<
        AnimationTemplate & { _source: string; _sourceDisplayName: string }
      > = [];
  ...
  const getTemplateOverlays = useCallback(
    async (
      params: { searchQuery?: string; page?: number; perPage?: number } = {},
      config?: Record<string, any>,
    ): Promise<MergedSearchResult<TemplateOverlay>> => {
      ...
      const templatePromises = activeAdaptors.map(async (adaptor) => {
        try {
          ...
          console.error(
            `Error fetching templates from ${adaptor.name}:`,
            error,
          );
          return {
            adaptor,
      ...
      // Merge and attribute results
      const mergedItems: Array<
        TemplateOverlay & { _source: string; _sourceDisplayName: string }
      > = [];
...
export const useMediaAdaptors = (): MediaAdaptorContextType => {
  ...
  if (!context) {
    throw new Error(
      "useMediaAdaptors must be used within MediaAdaptorProvider",
    );
  }
  return context;
};

...
export const useAlignmentGuides = ({
  ...
  const calculateGuidePositions = useCallback(
    (draggedOverlay: Overlay, allOverlays: Overlay[]): GuidePosition[] => {
  ...
  const findActiveGuides = useCallback(
    (draggedOverlay: Overlay, allOverlays: Overlay[]): GuidePosition[] => {
      ...
      const candidateGuides: Array<
        GuidePosition & { distance: number; alignmentType: string }
      > = [];
      ...
      allGuides.forEach((guide) => {
        ...
        const currentSnapThreshold = isCanvasEdge
          ? edgeSnapThreshold
          : snapThreshold;
        ...
        if (guide.x !== undefined) {
          ...
          if (
            distanceToRight <= currentSnapThreshold &&
          ...
          if (
            distanceToCenterX <= currentSnapThreshold &&
            distanceToCenterX < distance
          ) {
        ...
        if (guide.y !== undefined) {
          ...
          if (
            distanceToBottom <= currentSnapThreshold &&
          ...
          if (
            distanceToCenterY <= currentSnapThreshold &&
            distanceToCenterY < distance
          ) {
      ...
      const guidesByAlignmentAndPosition = new Map<
        string,
        typeof candidateGuides
      >();

      candidateGuides.forEach((guide) => {
        const position =
          guide.x !== undefined ? `x-${guide.x}` : `y-${guide.y}`;
      ...
      guidesByAlignmentAndPosition.forEach((guides) => {
        ...
        guides.sort((a, b) => {
          const aPriority = a.type.startsWith("canvas-center")
            ? 3
            : a.type.startsWith("canvas-edge")
              ? 2
              : 1;
          const bPriority = b.type.startsWith("canvas-center")
            ? 3
            : b.type.startsWith("canvas-edge")
              ? 2
              : 1;
    ...
    (
      ...
      activeGuides.forEach((guide) => {
        if (guide.x !== undefined) {
          ...
          const distanceToLeft = Math.abs(guide.x - draggedLeft);
          if (
            distanceToLeft < closestXDistance &&
            distanceToLeft <= currentSnapThreshold
          ) {
          ...
          const distanceToRight = Math.abs(guide.x - draggedRight);
          if (
            distanceToRight < closestXDistance &&
            distanceToRight <= currentSnapThreshold
          ) {
          ...
          const distanceToCenterX = Math.abs(guide.x - draggedCenterX);
          if (
            distanceToCenterX < closestXDistance &&
            distanceToCenterX <= currentSnapThreshold
          ) {
      ...
      activeGuides.forEach((guide) => {
        if (guide.y !== undefined) {
          ...
          const distanceToTop = Math.abs(guide.y - draggedTop);
          if (
            distanceToTop < closestYDistance &&
            distanceToTop <= currentSnapThreshold
          ) {
          ...
          const distanceToBottom = Math.abs(guide.y - draggedBottom);
          if (
            distanceToBottom < closestYDistance &&
            distanceToBottom <= currentSnapThreshold
          ) {
          ...
          const distanceToCenterY = Math.abs(guide.y - draggedCenterY);
          if (
            distanceToCenterY < closestYDistance &&
            distanceToCenterY <= currentSnapThreshold
          ) {

...
export const validateTiming = (
  ...
  if (
    ...
    endMs > captions[captionIndex + 1].startMs
  ) {
...
export const useCaptionTimeline = ({
  ...
  useEffect(() => {
    ...
    const activeIndex = localOverlay.captions.findIndex(
      (caption) =>
        relativeCurrentMs >= caption.startMs &&
        relativeCurrentMs < caption.endMs,
    );
  ...
  const handleTimingChange = (
    ...
    if (absoluteTime === -1) {
      setTimingErrors((prev) => ({
        ...prev,
        [captionIndex]: "Invalid time format (MM:SS or MM:SS.m)",
      }));
      return;
    }
    ...
    const words = updatedCaption.text
      .split(/\s+/)

...
export const useCaptions = () => {
  ...
  const parseTimeString = useCallback((timeString: string): number => {
    try {
      ...
      throw new Error(
        `Failed to parse time "${timeString}": ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }, []);
   ...
   */
  const validateSRTStructure = useCallback(
    (content: string): SRTParseError[] => {
      ...
      if (!timestampPattern.test(content)) {
        errors.push({
          type: "format",
          message:
            "No valid SRT timestamps found. Expected format: HH:MM:SS,mmm --> HH:MM:SS,mmm",
        });
      }
  ...
  const distributeWordTiming = useCallback(
    (words: string[], startMs: number, endMs: number): CaptionWord[] => {
   ...
   */
  const parseSRT = useCallback(
    async (content: string): Promise<ParseSRTResult> => {
      setState((prev) => ({
        ...prev,
        isProcessing: true,
        isError: false,
   ...
   */
  const createCaptionOverlay = useCallback(
    (captions: Caption[]): CaptionOverlay => {
  ...
  const handleFileUpload = useCallback(
    async (file: File): Promise<ParseSRTResult> => {
      if (!file) {

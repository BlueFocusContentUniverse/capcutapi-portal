...
export const useExtendedThemeSwitcher = ({
  ...
  const [currentTheme, setCurrentTheme] =
    useState<ExtendedThemeMode>(defaultTheme);
  const [isLoaded, setIsLoaded] = useState(false);

  // Combine default and custom themes
  const availableThemes = useMemo(
    () =>
      showDefaultThemes ? [...defaultThemes, ...customThemes] : customThemes,
    [showDefaultThemes, customThemes],
  );

  const applyTheme = useCallback(
    (themeId: ExtendedThemeMode) => {

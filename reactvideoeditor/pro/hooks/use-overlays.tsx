...
export const useOverlays = (
  ...
  useEffect(() => {
    if (
      ...
      !selectedOverlayIds.includes(selectedOverlayId)
    ) {
  ...
  useEffect(() => {
    if (
      ...
      selectedOverlayIds[0] !== selectedOverlayId
    ) {
      ...
      selectedOverlayIds.length > 1 &&
      selectedOverlayId !== selectedOverlayIds[0]
    ) {
      setSelectedOverlayId(selectedOverlayIds[0]);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
   ...
   */
  const applyInitialOverlaysIfNeeded = useCallback(
...
const createSplitOverlays = (
  ...
  const splitWaveformData = (
    originalWaveformData: any,
    ...
    const firstPeakCount = Math.floor(
      (firstDuration / totalDuration) * totalPeaks,
    );
    ...
    return [
      {
        ...
        length: Math.floor(
          (originalWaveformData.length || 0) * (firstDuration / totalDuration),
        ),
      },
      {
        peaks: secondHalfPeaks,
        length: Math.floor(
          (originalWaveformData.length || 0) * (secondDuration / totalDuration),
        ),
      },
    ];
  };

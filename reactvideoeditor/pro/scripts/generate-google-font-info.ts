...
async function generateFontInfo() {
  console.log("🚀 Starting Google Fonts data generation...");
  ...
  console.log(`\n📦 Loading ${top250.length} font modules...`);
  ...
  for (const font of top250) {
    try {
      ...
      successCount++;
      console.log(
        `✅ Loaded ${fontInfo.fontFamily} (${successCount}/${top250.length})`,
      );
    } catch (error) {
      errorCount++;
  ...
  console.log(
    `\n📊 Summary: ${successCount} fonts loaded successfully, ${errorCount} errors`,
  );
  ...
  console.log("\n📝 Generating google-fonts.ts (server-side database)...");
...
`;
  ...
  console.log(
    `✅ Created: ${serverFilePath} (${(serverFileContent.length / 1024 / 1024).toFixed(2)}MB)`,
  );

  // Generate google-fonts-list.ts (browser-safe)
  console.log("\n📝 Generating google-fonts-list.ts (browser-safe list)...");
...
`;
  ...
  console.log(
    `✅ Created: ${browserFilePath} (${(browserFileContent.length / 1024).toFixed(2)}KB)`,
  );
  ...
  try {
    ...
    console.warn(
      "⚠️  Prettier formatting failed, but files were generated successfully",
    );
  }

  console.log("\n✨ Font data generation complete!");

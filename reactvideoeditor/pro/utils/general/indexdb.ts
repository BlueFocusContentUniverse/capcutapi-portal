  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = (event) => {
      ...
      if (error && error.name === "VersionError") {
        console.error(
          "Database version conflict detected. The database may need to be cleared.",
        );
        console.error(
          "To fix this, you can call clearIndexedDB() from the browser console.",
        );
        ...
        deleteRequest.onerror = () => {
          reject(
            new Error("Could not clear IndexedDB to resolve version conflict"),
          );
        };
      } else {
 ...
 */
export const refreshMediaBlobUrl = async (
  id: string,
): Promise<string | null> => {
  try {
    const db = await initDB();
...
export const clearIndexedDB = (): Promise<void> => {
  return new Promise((resolve, reject) => {
    ...
    deleteRequest.onblocked = () => {
      console.warn(
        "IndexedDB deletion blocked. Please close all tabs using this application and try again.",
      );

  ...
  try {
    ...
    if (file.type.startsWith("video/") || file.type.startsWith("audio/")) {
      try {
        ...
        console.warn(
          "Failed to get duration with media-parser, falling back to legacy method:",
          error,
        );
    ...
    try {
      ...
      console.warn(
        "Server upload failed, falling back to blob storage:",
        serverError,
      );
...
export const generateThumbnail = async (file: File): Promise<string> => {
  if (file.type.startsWith("image/")) {
    ...
    try {
      const thumbnail = await MediaBunnyThumbnailService.generateThumbnail(
        file,
        {
          width: 320,
          height: 180,
      ...
      console.error(
        "MediaBunny thumbnail generation failed, falling back to canvas method:",
        error,
      );
...
export const getMediaDuration = async (
  ...
  if (file.type.startsWith("video/")) {
    ...
    try {
      ...
      console.error(
        "MediaBunny duration detection failed, falling back to HTML video element:",
        error,
      );

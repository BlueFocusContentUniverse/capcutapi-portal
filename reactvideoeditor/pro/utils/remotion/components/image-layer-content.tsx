...
import {
  layout3DTemplates,
  getLayout3DKey,
} from "../../../adaptors/default-3d-layout-adaptors";
...
export const ImageLayerContent: React.FC<ImageLayerContentProps> = ({
  ...
  const enterAnimation =
    !isExitPhase && overlay.styles.animation?.enter
      ? animationTemplates[
          getAnimationKey(overlay.styles.animation.enter)
        ]?.enter(frame, overlay.durationInFrames)
      : {};
  ...
  const exitAnimation =
    isExitPhase && overlay.styles.animation?.exit
      ? animationTemplates[
          getAnimationKey(overlay.styles.animation.exit)
        ]?.exit(frame, overlay.durationInFrames)
      : {};
  ...
  const layout3DTransform =
    overlay.styles.layout3D?.layout && overlay.styles.layout3D.layout !== "none"
      ? layout3DTemplates[
          getLayout3DKey(overlay.styles.layout3D.layout)
        ]?.transform()
      : {};
  ...
  const imageStyle: React.CSSProperties = {
    ...
    backgroundColor:
      overlay.styles.layout3D?.layout &&
      overlay.styles.layout3D.layout !== "none"
        ? "transparent"
        : "transparent",
    ...
    ...(currentAnimation?.opacity !== undefined && {
      opacity: currentAnimation.opacity,
    }),
    ...(currentAnimation?.filter !== undefined && {
      filter: currentAnimation.filter,
    }),
    ...(currentAnimation?.clipPath !== undefined && {
      clipPath: currentAnimation.clipPath,
    }),
  };
  ...
  const containerStyle: React.CSSProperties = {
    ...
    transition:
      "background-color 0.4s ease-out, border-radius 0.4s ease-out, border 0.4s ease-out, box-shadow 0.4s ease-out",
  };
  ...
  const processImageWithGreenscreen = useCallback(() => {
    ...
    const { drawX, drawY, drawWidth, drawHeight } =

...
export const TextLayerContent: React.FC<TextLayerContentProps> = ({
  ...
  const fontStyle = (overlay.styles.fontStyle || "normal") as
    | "normal"
    | "italic";
  ...
  const fontSize = useMemo(() => {
    ...
    const extractPadding = (paddingStr: string | undefined) => {
      ...
      if (values.length === 1) {
        ...
        return {
          vertical: values[0] + values[2],
          horizontal: values[1] + values[3],
        };
      }
    ...
    const availableWidth = Math.max(
      20,
      overlay.width - actualPaddingHorizontal - borderWidth * 2,
    );
    const availableHeight = Math.max(
      20,
      overlay.height - actualPaddingVertical - borderWidth * 2,
    );
    ...
    overlay.height,
    overlay.content,
    overlay.styles.padding,
    overlay.styles.border,
    overlay.styles.lineHeight,
    overlay.styles.fontSizeScale,
  ]);
